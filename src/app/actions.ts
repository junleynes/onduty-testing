...
export async function backupDatabase(): Promise<{ success: boolean; data?: string; filename?: string; error?: string }> {
    try {
        ...
        const db = getDb();
        // Checkpoint WAL so the backup file on disk is complete before we read it
        ...
        try { await writeAuditLog({ action: 'backup.admin_panel', detail: filename }); } catch (_) {}
...
export async function restoreDatabase(base64Data: string): Promise<{ success: boolean; error?: string }> {
    try {
        ...
        // 50 MB hard cap — this runs through a server action so Next.js already
        // enforces a body size limit upstream, but we double-check here too.
        const MAX_BYTES = 50 * 1024 * 1024;
        if (buffer.length > MAX_BYTES) {
            return { success: false, error: 'Backup file too large. Maximum size is 50 MB.' };
        }
        ...
        // Validate magic bytes — ZIP (PK) or raw SQLite3 file only
        const isZip    = buffer[0] === 0x50 && buffer[1] === 0x4B;
        const isSqlite = buffer.slice(0, 16).toString('utf8') === 'SQLite format 3\u0000';
        if (!isZip && !isSqlite) {
            return { success: false, error: 'Invalid file. Upload a .zip backup or a .db SQLite file.' };
        }
        ...
        if (isZip) {
            ...
            // Restore uploads — reject any entry attempting path traversal
            ...
            // Legacy: bare .db file, no uploads to restore
            fs.writeFileSync(dbPath, buffer);
        }

        try { await writeAuditLog({ action: 'restore.admin_panel' }); } catch (_) {}
